...
const int latchPin1 = 11;        // ST_CP 1
const int latchPin2 = 10;        // ST_CP 2
...
// Define Card struct and cards array
struct Card {
  byte uid[4];
  String name;
  long fees;
...
Card cards[] = {
  {{0x83, 0xB2, 0x55, 0x10}, "Mohamed Mekkawy", 10, 50},
  {{0x83, 0xB1, 0x12, 0x12}, "Saad Shallan", 10, 50}
};


String getCardUIDString(byte *uid, byte uidSize) {
  String uidString = "";
...
void checkLightSensor() {
    ...
    if (analogValue > 700) {
        Serial.println(" - Dark");
        turnOnLightRelay();
    } else if (analogValue < 700 && analogValue > 600) {
        Serial.println(" - Dim");
        turnOnLightRelay();
    } else {
...
void rfidLoop() {
    ...
    String cardUIDString = getCardUIDString(uid, uidSize);
    Serial.print("Card UID: ");
    Serial.println(cardUIDString);

    bool cardFound = false;
    for (int i = 0; i < sizeof(cards) / sizeof(Card); i++) {
        if (memcmp(uid, cards[i].uid, uidSize) == 0) {
            cardFound = true;
            if (cards[i].balance < cards[i].fees) {
                lcd.setCursor(0, 1);
                ...
                lcd.setCursor(0, 1);
                lcd.print("Balance: LE ");
                lcd.print(cards[i].balance);
                delay(2000);

                cards[i].balance -= cards[i].fees;
